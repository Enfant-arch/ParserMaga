# - *- coding: utf- 8 - *-
from utils.db_api.psql  import get_userx, get_settingsx
...
def get_user_profile(user_id):
    ...
    if (get_user[4] or settings[4] == True):
        attempt = True
    if attempt == True: attempt = "есть"
    if attempt == False: attempt = "нет"
    msg = f"<b>👤 Ваш профиль:</b>\n" \
          f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n" \
          f"🔑 Мой ID: <code>{get_user[1]}</code>\n" \
          f"📇 Логин: <b>@{get_user[2]}</b>\n" \
          f"📅 Регистрация: <code>{get_user[7]}</code>\n" \
          f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n" \
          f"💳 Доступ к парсеру: <code>{attempt}</code>\n"
    return msg
...
def search_user_profile(user_id):
    ...
    attempt = False
    settings = get_settingsx()
    if (get_status_user[4] or settings[4] == True):
        attempt = True
    if attempt == True: attempt = "есть"
    if attempt == False: attempt = "нет"
