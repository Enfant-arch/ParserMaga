...
def add_userx(user_id, user_login, user_name, balance, all_refill, reg_date):
    ...
    cursor.execute("INSERT INTO storage_users "
                "(user_id,  user_login, user_name, is_prime, active_user, balance, all_refill, reg_date ) "
                "VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
                [user_id, user_login, user_name, False, True, balance, all_refill, reg_date])
...
def get_paymentx():
    cursor.execute("SELECT * FROM storage_payment")
    return cursor.fetchone()
...
def create_bdx():
        cursor.execute("CREATE TABLE IF NOT EXISTS storage_users("
                       ...
                       "user_id BIGINT, user_login TEXT, user_name TEXT, is_prime BOOL, active_user BOOL, "
                       "balance DECIMAL, all_refill INTEGER, reg_date TIMESTAMP)")
        logging.info("Setuping storage_users (1/2) | Creating...")
       
    

        cursor.execute("CREATE TABLE IF NOT EXISTS storage_settings("
                    "contact TEXT UNIQUE, faq TEXT, "
                    "status bool, status_buy bool, parser_open bool,"
                    "profit_buy INT, profit_refill INT)")
        try:
            ...
            parameters = ["ℹ Контакты. Измените их в настройках бота.\n"
                            ...
                            "➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                            f"{bot_description}",
                            "True", "True", "True", now_unix, now_unix]
            cursor.execute("INSERT INTO storage_settings(" \
                    "contact, faq, status, status_buy, parser_open, profit_buy, profit_refill) " \
                    "VALUES (%s, %s, %s, %s, %s, %s, %s)", parameters)
        except psycopg2.errors.UniqueViolation:
             pass
   
        logging.info("Configure storage_settings (2/2) | Creating...")
